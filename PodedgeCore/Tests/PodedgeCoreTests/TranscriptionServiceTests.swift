...
struct MLXLLMProviderTests {
    ...
    @Test("complete throws stub error")
    func completeThrows() async {
        let provider = MLXLLMProvider()
        do {
            _ = try await provider.complete(prompt: "test", systemPrompt: nil, maxTokens: 100)
            Issue.record("Expected stub to throw")
        } catch let error as PodedgeError {
            guard case .llmFailed(let reason) = error else {
                Issue.record("Expected .llmFailed, got \(error)")
                return
            }
            #expect(reason.contains("mlx-swift"))
        } catch {
            Issue.record("Unexpected error type: \(error)")
        }
    }

    @Test("schema complete throws stub error")
    func schemaCompleteThrows() async {
        let provider = MLXLLMProvider()
        do {
            _ = try await provider.complete(prompt: "test", systemPrompt: nil, maxTokens: 100, schema: "{}")
            Issue.record("Expected stub to throw")
        } catch let error as PodedgeError {
            guard case .llmFailed = error else {
    ...
    @Test("stream finishes with stub error")
    func streamThrows() async {
        let provider = MLXLLMProvider()
        let stream = provider.stream(prompt: "test", systemPrompt: nil, maxTokens: 100)
        do {
            for try await _ in stream {
                Issue.record("Expected no chunks from stub")
            }
